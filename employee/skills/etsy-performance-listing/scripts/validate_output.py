...
def _safe_text(value: Any, field: str, issues: list[dict[str, str]], *, nonempty: bool = True) -> str | None:
...
def _positive_int(rules: dict[str, Any], name: str, default: int, issues: list[dict[str, str]]) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        issues.append({"field": f"rules.{name}", "message": "must be a positive integer"})
        return default
    return value
...
def validate_generated(payload: Any, rules: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    title = _safe_text(payload.get("head_titles"), "head_titles", issues)
    if title:
    ...
    cleaned: dict[str, Any] = {
        "head_titles": title,
        "tags": cleaned_tags,
        "specification": _safe_text(payload.get("specification"), "specification", issues),
        "category": _safe_text(payload.get("category"), "category", issues),
        "instructions_for_buyers": _safe_text(payload.get("instructions_for_buyers"), "instructions_for_buyers", issues),
    }
    ...
    for field in ("fact_warnings", "quality_warnings"):
        ...
        if not isinstance(values, list):
            ...
            cleaned[field] = [_safe_text(value, f"{field}[{index}]", issues) for index, value in enumerate(values)]
    cleaned["rule_version"] = _safe_text(payload.get("rule_version"), "rule_version", issues)
