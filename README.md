...
第一版使用本机单用户 SQLite，不包含 Etsy 自动发布、在线多用户部署、后台浏览器采集、AI 图片生成、宏工作簿（`.xlsm`）、旧版 `.xls`、移动端原生应用或无人审核的自动知识升级。Excel 单文件上限为 200 MB；原创保护是启发式相似度护栏，不能替代人工审核或法律判断。迁移包不包含模型凭据。
