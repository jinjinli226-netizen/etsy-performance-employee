...
> 发布状态：截至 2026-08-21，本机生产路径已激活并通过真实 38 行图片工作簿验收，员工 Profile 校验通过。新建任务使用 `mvp-default-v3` 的结构化 Specification（固定大标题 + 4 条不重复 Emoji 卖点）；既有 v2 任务保持兼容。跨电脑部署请另见 [网站与数字员工部署迁移指南](网站与数字员工部署迁移指南.md)。
...
大工作簿包含大量内嵌图片时可能运行十几分钟。Excel 页面只要进度仍在更新就应继续等待；当前版本的事件流没有 190 秒绝对前端超时。若升级后仍看到“连接数字员工超时”，先强制刷新页面以加载最新前端，再检查 `/api/health` 和任务详情；不要在旧任务仍运行时反复上传同一文件。
...
- 业务数据使用本机 SQLite；Excel 单文件上限为 200 MB；
